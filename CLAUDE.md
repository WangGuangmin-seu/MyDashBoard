...
- **电解铝模拟冶炼毛利**（`metals.aluminum.sim_margin`，元/吨，日频）：云南电价本身无干净
  公开源（akshare 只有全国用电量非省级电价；昆明电力交易中心月度公告的成交价只在文字里，
  无结构化接口），故不追踪电价本身，改为从财报反推冶炼环节的**综合转换成本残差 C**：
  `C = 铝价(报告期均) × (1 − 毛利率) − 1.93 × 氧化铝价(报告期均)`，
  `模拟单吨毛利(日) = 铝价(日) − 1.93×氧化铝价(日) − C`。
  1.93 = 氧化铝单耗（吨氧化铝/吨电解铝，行业经验值）。毛利率取云铝股份（`SZ000807`）
  「有色金属冶炼业」分部毛利率，来自东方财富 F10 `emweb.securities.eastmoney.com/PC_HSF10/
  BusinessAnalysis/PageAjax?code=SZ000807`（无 key），字段 `zygcfx[].GROSS_RPOFIT_RATIO`，
  筛 `MAINOP_TYPE=="1"` 且 `ITEM_NAME` 含"冶炼"，取 `REPORT_DATE` 最新一条。
  **实测**（2026-08-10）：毛利率 0.181588，报告期 2025-12-31（年报），模拟毛利 ≈7880 元/吨。
  C 只随财报半年更新一次（慢变量），日频波动完全由铝价/氧化铝价两个日频序列驱动，信号干净；
  但 C 本身混合了电力+阳极+辅料+折旧+制造费用，**不是纯电价**，口径上是"冶炼毛利"而非电价。
  之前尝试过的人工录入云南电价（`manual` 采集器）已废弃并移除。
